...
import { CreateHolidayInstancePayload, HolidayInstance } from '../../types/redux/holidays';
...
export default function HolidayInstancePage() {
	...
	const holidayName = (id: number) =>
		holidays.find(h => h.id === id)?.name ?? translate('holiday.base.unknown');
	...
	const holidayOptions = useMemo(
		() => sortedHolidays.map(h => ({ value: h.id, label: h.name })),
		[sortedHolidays]
	);
